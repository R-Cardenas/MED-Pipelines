...
process cgpMAP {
  ...
  input:
  file reads from read2_ch
  output:
  file "${reads[0]}.bam" into cgp_ch
  script:
  """
	rm -fr $baseDir/output/cgpMAP/${reads[0]} # delete and remake path in case of retry pipeline
	mkdir -p $baseDir/output/cgpMAP/${reads[0]}

  ds-cgpmap.pl  \
  -outdir $baseDir/output/cgpMAP/${reads[0]} \
  -r $cgpmap_genome \
  -i $cgpmap_index \
  -t 10 \
  ${reads[0]} ${reads[1]}

	mv $baseDir/output/cgpMAP/${reads[0]}/*.bam \
	$baseDir/output/cgpMAP/${reads[0]}/${reads[0]}.bam

	echo 'fq1: ${reads[0]} fq2: ${reads[1]} bam_name: ${reads[0]}.bam' >> $baseDir/logs/${projectname}_cgpmap_samples.log
	ls -l  ${reads[0]} >> $baseDir/logs/symbolic_test_fastq.log
	ls -l  ${reads[1]} >> $baseDir/logs/symbolic_test_fastq.log
  """
}
