...
process cgpMAP {
  ...
  executor 'slurm'
	...
	storeDir "$baseDir/outputCOPY3/cgpMAP/${reads[0]}"
  input:
  ...
  rm -fr $baseDir/outputCOPY3/cgpMAP/${reads[0]} # delete and remake path in case of retry pipeline
	mkdir -p $baseDir/outputCOPY3/cgpMAP/${reads[0]}

  name=\$(echo '${reads[0]}' | sed -e 's/.*[/]//' -e 's/-.*//')
  BaseName=\$(basename ${reads[1]})

  ds-cgpmap.pl \
  -outdir $baseDir/outputCOPY3/cgpMAP/${reads[0]} \
  -r $cgpmap_genome \
  -i $cgpmap_index \
  -t 10 \
  -s \$name \
