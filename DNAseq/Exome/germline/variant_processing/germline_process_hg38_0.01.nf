...
process ped {
  errorStrategy { sleep(Math.pow(2, task.attempt) * 200 as long); return 'retry' }
	maxRetries 6
	executor 'local'
  storeDir "$baseDir/output_0.01/VCF_collect/merge_vcf/indels/caller"
  input:
...
process count_variants{
  errorStrategy { sleep(Math.pow(2, task.attempt) * 200 as long); return 'retry' }
	maxRetries 6
	executor 'local'
  storeDir "$baseDir/output_0.01/variant_counts/indels/caller_merge"
  input:
...
process merge_family_indels {
  errorStrategy { sleep(Math.pow(2, task.attempt) * 200 as long); return 'retry' }
	maxRetries 6
	executor 'local'
  storeDir "$baseDir/output_0.01/VCF_collect/merge_vcf/indels/family"
  input:
...
process indels_filter {
  errorStrategy { sleep(Math.pow(2, task.attempt) * 200 as long); return 'retry' }
	maxRetries 6
	executor 'local'
  storeDir "$baseDir/output_0.01/VCF_collect/merge_vcf/indels/family/filter"
  input:
...
process count_variants2{
  errorStrategy { sleep(Math.pow(2, task.attempt) * 200 as long); return 'retry' }
	maxRetries 6
	executor 'local'
  storeDir "$baseDir/output_0.01/variant_counts/indels/family_merge"
  input:
...
//  errorStrategy { sleep(Math.pow(2, task.attempt) * 200 as long); return 'retry' }
//	maxRetries 6
  storeDir "$baseDir/output_0.01/VCF_collect/VEP/vcf"
  input:
...
process slivar2 {
  ...
  executor 'slurm'
	...
	queue 'hmem-512'
	memory { 130.GB * task.attempt }
  storeDir "$baseDir/output_0.01/VCF_collect/VEP/slivar"
  input:
...
//  errorStrategy { sleep(Math.pow(2, task.attempt) * 200 as long); return 'retry' }
//	maxRetries 6
  storeDir "$baseDir/output_0.01/VCF_collect/VEP/tsv/indels"
  input:
...
process merge_caller_snps {
  errorStrategy { sleep(Math.pow(2, task.attempt) * 200 as long); return 'retry' }
	maxRetries 6
  storeDir "$baseDir/output_0.01/VCF_collect/merge_vcf/snps/caller"
  input:
...
process count_variants3{
  ...
  storeDir "$baseDir/output_0.01/variant_counts/snps/caller_merge"
  input:
...
process merge_fam_snps{
  errorStrategy { sleep(Math.pow(2, task.attempt) * 200 as long); return 'retry' }
	...
	executor 'local'
	memory { 7.GB * task.attempt }
  storeDir "$baseDir/output_0.01/VCF_collect/merge_vcf/snps/family"
  input:
...
process snps_filter {
  errorStrategy { sleep(Math.pow(2, task.attempt) * 200 as long); return 'retry' }
	...
	executor 'local'
	memory { 7.GB * task.attempt }
  storeDir "$baseDir/output_0.01/VCF_collect/merge_vcf/snps/family/filtered"
  input:
...
process count_variants4{
  errorStrategy { sleep(Math.pow(2, task.attempt) * 200 as long); return 'retry' }
	...
	executor 'local'
	memory { 7.GB * task.attempt }
  storeDir "$baseDir/output_0.01/variant_counts/snps/family_merge"
  input:
...
//	maxRetries 6
  ...
  --fasta /var/spool/mail/cgpwgs_ref/GRCh38/core_ref_GRCh38_hla_decoy_ebv/genome.fa \
  --custom /var/spool/mail/conservation/hg38.phastCons7way.bw,Conservation,bigwig,exact \
  --gff /var/spool/mail/ensembl_gff_GRch38/gencode.v38.chr_patch_hapl_scaff.annotation.sorted.gff3.gz \
  --show_ref_allele \
  --use_given_ref \
  --verbose \
  --check_ref \
...
//  errorStrategy { sleep(Math.pow(2, task.attempt) * 200 as long); return 'retry' }
//	maxRetries 6
  storeDir "$baseDir/output_0.01/VCF_collect/VEP/tsv/snps"
  input:
...
process Rtables {
	executor 'local'
  storeDir "$baseDir/output_0.01/variant_counts/data"
  input:
...
//	maxRetries 6
  ...
  output:
  val "$baseDir/output_0.01/markdown/${projectname}_report.html"
  script:
  """
  mkdir -p $baseDir/output_0.01/markdown

  Rscript -e 'rmarkdown::render("$baseDir/bin/R/germlineExomeTemplate.Rmd",
  output_file="$baseDir/output_0.01/markdown/${projectname}_report.html",
